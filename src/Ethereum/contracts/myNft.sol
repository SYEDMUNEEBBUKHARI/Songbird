...
contract TNFT is ERC721Enumerable, Ownable {
  ...
  uint256 public maxMintAmount = 165;
  ...
  function _baseURI() internal view virtual override returns (string memory) {
   ...
   winNft(winCount);
    ...
    function findId(uint findIdCount) internal{
      uint256 winningId =  block.timestamp % (max - min + 1) + min;
      if(collectId.length == findIdCount)
      {
          return ;
      }
      if(!transferedNft[winningId])
      {
        collectId.push(winningId);
        transferedNft[winningId]=true;
        return;
      }
